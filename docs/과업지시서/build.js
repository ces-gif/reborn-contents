...
const cover = [
  ...
  P('학생창업 경진대회 및 창업 특강 운영 용역', { align: AlignmentType.CENTER, bold: true, size: 30, after: 1600 }),
  coverInfo,
...
const s2 = [
  ...
  table([2400, 1750, 5204], [
    ...
    R({ t: '예선 접수 마감' }, ctr('10.13(화) 18:00'), '기획서 + 프로토타입 데모 링크 제출 마감'),
...
const s3 = [
  ...
  table([3200, 1200, 4954], [
    R('심사 항목', '배점', '평가 내용'),
    R({ t: '프로토타입 구현 완성도', bold: true }, ctr('30%'), '바이브 코딩으로 실제 동작하는 결과물을 구현하였는가'),
...
const s4 = [
  // 예산표가 쪽을 걸쳐 잘리면 세로 병합한 '구분' 칸이 뒷장에서 빈칸으로 보인다
  ...
  table([1750, 2200, 3604, 1800], [
    ...
    R(MERGE, '장려상 (4~10등)', '200,000원 상당 상품 × 7팀', won('1,400,000')),
    R({ t: '전문가 활용비', ...VM }, '창업 특강 강사료', '바이브 코딩 실습 특강 외부 전문 강사 (2시간 기준)', won('500,000')),
    R(MERGE, '1:1 창업 멘토링비', '100,000원 × 10개 팀', won('1,000,000')),
    R(MERGE, '예선 서류 평가료', '150,000원 × 2명 (내/외부 위원)', won('300,000')),
    R(MERGE, '결승 심사수당', '현직 VC 3명 × 500,000원', won('1,500,000')),
    R({ t: '실습 환경비', bold: true, align: AlignmentType.CENTER }, 'AI 코딩 도구 이용료', '특강·멘토링 실습용 AI 코딩 도구 계정 및 크레딧 10,000원 × 30팀', won('300,000')),
    R({ t: '홍보 및 인쇄비', bold: true, align: AlignmentType.CENTER }, '포스터/자료집 등', '포스터, 현수막, 자료집, 상장 인쇄비', won('700,000')),
    R({ t: '행사 운영비 및 일반관리비', bold: true, align: AlignmentType.CENTER }, '식대/기념품/대행수수료', '당일 식대·다과, 기념품, 소모품, 현장 운영 인력 및 대행 수수료', won('2,500,000')),
  ...
  callout('※ 원 과업지시서 대비 「행사 운영비 및 일반관리비」에서 300,000원을 조정하여 「실습 환경비」를 신설하였으며, 총 예산 10,000,000원(부가가치세 포함)은 변동이 없다.'),
];
      ...
      headers: {
        default: new Header({
          children: [new Paragraph({
            ...
            children: [run('한국항공대학교 바이브 코딩 기반 학생창업 경진대회 및 창업 특강 운영 용역', { size: 16, color: GRAY })],
          })],
        }),
      },
      footers: {
