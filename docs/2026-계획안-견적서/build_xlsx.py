...
# ─────────────────────────────────────────── 강사수당 지급기준 (발주처 기준표 원문)
ws5 = wb.create_sheet('강사수당 지급기준')
widths(ws5, {'A': 8, 'B': 52, 'C': 34, 'D': 11, 'E': 11, 'F': 13})
ws5.merge_cells('A1:F1')
put(ws5, 'A1', '3. 강사수당 지급기준 —  가. 일반강의 강사수당', sz=12, b=True, align='center', border=False)
put(ws5, 'F2', '(단위 : 원)', align='right', border=False)

put(ws5, 'A3', '등급', b=True, fill=HEAD, align='center')
put(ws5, 'B3', '적용대상 (청탁금지법 적용 유무로 구분)', b=True, fill=HEAD, align='center')
put(ws5, 'C3', '', fill=HEAD)
put(ws5, 'D3', '최초 1시간', b=True, fill=HEAD, align='center')
put(ws5, 'E3', '초과 매시간', b=True, fill=HEAD, align='center')
put(ws5, 'F3', '2시간 환산', b=True, fill=HEAD, align='center')
put(ws5, 'A4', '', fill=HEAD)
put(ws5, 'B4', '일반', b=True, fill=HEAD, align='center')
put(ws5, 'C4', '공직자 등', b=True, fill=HEAD, align='center')
for c in 'DEF':
    put(ws5, f'{c}4', '', fill=HEAD)
ws5.merge_cells('B3:C3')
for c in ('A', 'D', 'E', 'F'):
    ws5.merge_cells(f'{c}3:{c}4')

등급표 = [
    ('특1급',
     '• 전직 장관(급) 및 광역자치단체장\n• 전직 국회의원\n• 전직 한국은행장\n• 전직 대학의 총장(이사장)\n• 대기업 총수(부회장 이상)',
     '• 장관(급)*, 광역자치단체장*\n• 국회의원*\n• 한국은행장*\n• 대학의 총장(이사장)', 400_000, 300_000),
    ('특2급',
     '• 전직 차관(급) 및 기초자치단체장\n• 전직 공직유관단체장\n• 전직 전문대학 등의 총장(이사장)\n• 대기업 임원, 중견기업 대표',
     '• 차관(급), 기초자치단체장\n• 공직유관단체장\n• 전문대학 등의 총장(이사장)\n• 대형언론사 총수', 300_000, 200_000),
    ('가급',
     '• 전직 4급 이상 공무원\n• 전직 지방의회의원(의장 포함)\n• 전직 공직유관단체 임원(출연연구기관 부연구위원 이상)\n'
     '• 전직 대학의 교수(조교수 이상)\n• 유명 예술인·종교인, 이와 유사 분야의 유명 인사\n'
     '• 대기업 부장(급), 중견기업 임원, 중소기업 대표 (이에 준하는 기관·단체 적용 가능)\n'
     "• '전문직'으로서 3년 이상 실무경력자\n• 박사학위 취득 후 해당분야 3년 이상 실무경력자 (취미·소양·외국어·전산 강사 제외)\n"
     '• 국가대표 지도자 및 국가대표 출신 강사',
     '• 4급 이상 공무원\n• 지방의회의원(의장 포함)\n• 공직유관단체 임원(출연연구기관 부연구위원 이상)\n'
     '• 대학의 교수(조교수 이상)\n• 초·중·고교의 장(이사장)\n• 대형언론사 임원\n• 기타언론사 대표', 280_000, 120_000),
    ('나급',
     '• 전직 5급(상당) 공무원\n• 중견기업 부장(급), 중소기업 임원 (이에 준하는 기관·단체 적용 가능)\n'
     '• 원어민 어학 강사(외국국적을 가진 자로서 해당 국가에서 고등교육을 이수한 자)\n'
     '• 국가전문자격증을 가진 자로서 3년 이상 실무경력자\n• 기타 전문자격을 가진 자로서 5년 이상 실무경력자',
     '• 5급 이하 공무원\n• 공직유관단체 직원\n• 대학의 강사 등(교직원 포함)\n• 초·중·고교의 교직원\n• 기타언론사 임원',
     170_000, 80_000),
    ('다급',
     '• 전직 6급 이하 공무원\n• 외국어, 전산 등 강사\n'
     '• 체육, 레크리에이션 등 취미·소양 강사로서 해당분야 3년 이상(공공기관·공공교육훈련기관 경력) 또는 5년 이상(외부경력 포함) 강의 경력자',
     '', 100_000, 50_000),
    ('라급', '• 체육, 레크리에이션 등 취미·소양 강사', '', 80_000, 40_000),
    ('마급', '• 각종 교육운영(실기실습 등) 보조자', '', 60_000, 30_000),
]
r = 5
for 급, 일반, 공직자, 최초, 초과 in 등급표:
    선택 = 급 == '나급'
    fill = 'FFF2CC' if 선택 else ('F4F7FB' if r % 2 == 0 else None)
    put(ws5, f'A{r}', 급, b=True, align='center', fill=fill)
    put(ws5, f'B{r}', 일반, sz=8, wrap=True, fill=fill)
    put(ws5, f'C{r}', 공직자, sz=8, wrap=True, fill=fill)
    put(ws5, f'D{r}', 최초, align='right', fmt=WON, fill=fill)
    put(ws5, f'E{r}', 초과, align='right', fmt=WON, fill=fill)
    put(ws5, f'F{r}', f'=D{r}+E{r}', align='right', fmt=WON, b=선택, fill=fill)
    ws5.row_dimensions[r].height = max(30, 11 * (일반.count('\n') + 1))
    r += 1

for note in [
    '1. 전문직 : 감정평가사, 건축사, 공인노무사, 공인회계사, 관세사, 기술사, 법무사, 변리사, 변호사, 보험계리사, 세무사, 약사, 의사',
    '2. 특1급 초과 매시간은 300,000원이나, * 표시된 청탁금지법 적용 대상(공직자 등)은 200,000원을 적용한다.',
    '3. 2시간 환산 = 최초 1시간 + 초과 1시간. 본 과업의 창업 특강(2시간)은 나급을 적용하여 170,000 + 80,000 = 250,000원으로 계상하였다.',
    '4. 2025년 원가계산서도 동일하게 나급을 적용하였다 (4시간 17+8+8+8 = 410,000원 / 3시간 17+8+8 = 330,000원).',
    '5. 강사가 중소기업 대표 등 가급 요건에 해당하면 2시간 400,000원까지 적용할 수 있다.',
]:
    ws5.merge_cells(f'A{r}:F{r}')
    put(ws5, f'A{r}', note, sz=9, wrap=True, border=False)
