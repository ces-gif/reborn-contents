...
**갈래 B — 내 계정 OAuth 로 우회 (콘솔 정책을 안 건드림) ⭐ 설치 마법사 있음**

서비스 계정 키 없이 은성님 계정 권한으로 드라이브를 씁니다.
**폴더 공유도 필요 없습니다** (이미 은성님 폴더니까요).

설치 마법사가 콘솔 페이지 열기·토큰 발급·연결 검증까지 다 해줍니다.
윈도우면 저장소 폴더의 **`setup.bat` 을 더블클릭**하거나, 터미널에서:
...
| 단계 | 마법사가 하는 일 | 은성님이 하실 일 |
| --- | --- | --- |
| 1 | Drive API 페이지를 열어줌 | [사용] 버튼 클릭 |
| 2 | OAuth 동의 화면을 열어줌 | User Type 을 **내부(Internal)** 로 선택 |
| 3 | 사용자 인증 정보 페이지를 열어줌 | **데스크톱 앱** 클라이언트 만들고 값 2개 붙여넣기 |
| 4 | 구글 로그인 창을 띄우고 토큰 발급 | 로그인 후 [허용] 클릭 |

그다음 마법사가 **실제로 드라이브에 붙어서** 리퍼·새상품 폴더를 읽고,
`콘텐츠 발행` 폴더에 쓸 수 있는지, 로고를 가져올 수 있는지까지 검사합니다.
전부 통과하면 `google-secrets.txt` 에 넣을 값 3개를 적어주고 깃허브 시크릿 페이지를 열어줍니다.

> ⚠️ 2단계에서 **반드시 '내부'** 를 고르세요. '외부/테스트' 로 두면
> **리프레시 토큰이 7일 만에 만료**돼서 매주 다시 해야 합니다.
> ⚠️ `google-secrets.txt` 는 등록이 끝나면 지우세요. 채팅창에 붙여넣지 마세요.

토큰만 따로 다시 뽑고 싶으면 `python scripts/make_refresh_token.py` 를 쓰면 됩니다.
...
카드 등록 없이 무료 등급으로 쓸 수 있습니다.

1. <https://aistudio.google.com/apikey> 접속 (구글 계정으로 로그인)
2. **`API 키 만들기`** → 프로젝트 선택(기존 것 그대로 두셔도 됩니다) → 생성
3. 나온 키를 복사 → 깃허브 시크릿 **`GEMINI_API_KEY`**

무료 등급 한도는 분당 요청 수와 하루 요청 수로 걸립니다.
상품 15개면 하루 50건쯤 쓰므로 한도에 한참 못 미칩니다.

> **알아두실 것**: 무료 등급에서는 구글이 입력을 모델 개선에 쓸 수 있습니다.
> 상품 사진과 가격표가 구글로 갑니다. 이게 곤란하시면 유료 등급으로 올리거나
> (같은 키 그대로, 구글 클라우드 결제만 연결) 갈래 2를 쓰세요.
> **유료 등급에서는 학습에 쓰지 않습니다.**

### 갈래 2 — Claude (유료, 판독이 조금 더 정확)

1. <https://console.anthropic.com/> 로그인 → **API Keys → Create Key**
2. `sk-ant-...` 키를 복사 → 깃허브 시크릿 **`ANTHROPIC_API_KEY`**
3. **Billing** 에서 결제수단 등록 + 충전 (**무료 등급이 없습니다**)
4. `config/settings.yaml` 의 `model` 을 이렇게 바꿉니다:
   ```yaml
   model:
     provider: "anthropic"
...
> 두 키를 다 넣고 `provider: "auto"` 로 두면 제미나이를 먼저 씁니다.
